...
DEFAULT_PORTFOLIO_RELEASE_HISTORY = "data/portfolio/release_history.jsonl"
...
def _build_report(
    *,
    ...
    blocking_repos.sort(key=lambda row: (row["repo_id"], row["repo_root"], row["error_code"]))
    ...
    return {
        ...
        "task": str(latest_payload_from_history.get("task", "release")),
        "history": {
...
def render_portfolio_release_markdown(report: dict[str, Any]) -> str:
    ...
    if isinstance(transitions, list) and transitions:
        ...
        lines.append("- None")
    lines.append("")
    lines.append("## Blocking Repos")
    blocking = report.get("blocking_repos", [])
...
def run_portfolio_release_report(
    *,
    ...
    report = _build_report(
        command="portfolio_release_report",
        history_path=history_path,
...
def write_portfolio_release_outputs(report: dict[str, Any], *, json_path: str | None, md_path: str | None) -> None:
    ...
    if md_path:
        ...
        p.parent.mkdir(parents=True, exist_ok=True)
        p.write_text(render_portfolio_release_markdown(report), encoding="utf-8")
